...
public static class CourseScheduleParser
{
        public static async Task<List<CourseInfo>> ParseScheduleHtmlAsync(string htmlContent)
        {
            ...
            try
            {
                ...
                foreach (Match match in scriptMatches)
                {
                    ...
                    {
                        Console.WriteLine($"解析课程活动时出错: {ex.Message}");
                    }
                }
                ...
                if (courses.Count == 0)
                {
                    Console.WriteLine("未从脚本中找到课程，使用备用解析方法");
                ...
                Console.WriteLine($"成功解析 {courses.Count} 门课程信息");
                return courses;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"解析课表HTML时出错: {ex.Message}");
                return courses;
            }
        }
        /// <summary>
        /// 处理连续节次并格式化
        /// </summary>
        public static void ProcessContinuousPeriods(List<CourseInfo> courses)
        {
            Console.WriteLine("开始处理连续节次...");
            ...
            foreach (var course in courses)
            {
                try
                {
                    ...
                    if (course.Periods.Count > 0)
                    {
                        ...
                        Console.WriteLine($"课程 '{course.CourseName}' ({course.DayOfWeek}): 节次={string.Join(",", course.Periods)}, 格式化为 '{course.FormattedPeriods}'");
                    }
                    else
                    {
                        ...
                        Console.WriteLine($"课程 '{course.CourseName}' 没有节次信息");
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"处理课程 '{course.CourseName}' 的连续节次时出错: {ex.Message}");
                }
            }
            
            Console.WriteLine($"处理连续节次完成，共处理 {courses.Count} 门课程");
        }
        ...
        public static List<List<int>> GroupConsecutivePeriods(List<int> periods)
        {
        ...
        public static string FormatPeriodGroups(List<List<int>> periodGroups)
        {
        ...
        /// 检查列表是否连续
        /// </summary>
        public static bool IsSequentialList(List<int> list)
        {
            // 如果只有一个元素，不算连续
        ...
        /// 格式化节次信息
        /// </summary>
        public static string FormatPeriods(List<int> periods, bool isContinuous)
        {
        ...
        /// 格式化教师名称，处理特殊情况
        /// </summary>
        public static string FormatTeacherName(string rawName)
        {
        ...
        /// 解析教师ID
        /// </summary>
        public static Task ParseTeacherIdsAsync(List<CourseInfo> courses, string htmlContent)
        {
            try
            {
                HtmlDocument doc = new HtmlDocument();
                ...
                foreach (var course in courses)
                {
                        ...
                        courseToCourseInfoMap.TryGetValue(course.CourseNumber, out var tableInfo))
                    {
                        ...
                        if (!string.IsNullOrEmpty(tableInfo.TeacherName))
                        {
                            ...
                            if (teacherIdMap.TryGetValue(tableInfo.TeacherName, out int id))
                            {
                                course.TeacherId = id;
                                matched = true;
                                Console.WriteLine($"通过课程序号+表格教师名匹配: {course.CourseName}, 教师={course.TeacherName}, ID={id}");
                            }
                        }
                    }
                    ...
                    if (!matched && !string.IsNullOrEmpty(course.TeacherName))
                    {
                        if (teacherIdMap.TryGetValue(course.TeacherName, out int id))
                        {
                            course.TeacherId = id;
                            matched = true;
                            Console.WriteLine($"直接匹配教师名: {course.TeacherName}, ID={id}");
                        }
                        else
                        {
                            // 尝试按逗号分割的第一个教师名
                                ...
                                teacherIdMap.TryGetValue(firstTeacherName, out int firstId))
                            {
                                ...
                                matched = true;
                                Console.WriteLine($"匹配第一个教师名: {firstTeacherName}, ID={firstId}");
                            }
                            else
                            {
                                // 最后尝试提取纯中文名
                                ...
                                if (!string.IsNullOrEmpty(chineseName) && teacherIdMap.TryGetValue(chineseName, out int chineseId))
                                {
                                    ...
                                    matched = true;
                                    Console.WriteLine($"匹配纯中文名: {chineseName}, ID={chineseId}");
                                }
                            }
                        }
                    }
                ...
                Console.WriteLine($"教师ID匹配完成: 共{courses.Count}门课程，成功匹配{matchCount}门");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"解析教师ID时出错: {ex.Message}");
            }
        ...
        /// </summary>
        public static Dictionary<string, int> ExtractTeacherIds(string html)
        {
            ...
            try
            {
                ...
                foreach (Match match in directMatches)
                {
                    if (match.Success && match.Groups.Count >= 3)
                    {
                        ...
                        {
                            teacherMap[name] = id;
                            Console.WriteLine($"直接从teachers变量匹配到教师: {name} (ID: {id})");
                        }
                    }
                }
                ...
                if (teacherMap.Count == 0)
                {
                    ...
                    foreach (Match match in generalMatches)
                    {
                        if (match.Success && match.Groups.Count >= 3)
                        {
                            ...
                            {
                                teacherMap[name] = id;
                                Console.WriteLine($"正则匹配到教师: {name} (ID: {id})");
                            }
                        }
                    }
                }
                            ...
                            if (match.Success && match.Groups.Count > 1)
                            {
                                ...
                                foreach (Match item in teacherItems)
                                {
                                    if (item.Success && item.Groups.Count >= 3)
                                    {
                                        ...
                                        {
                                            teacherMap[name] = id;
                                            Console.WriteLine($"从数组提取教师: {name} (ID: {id})");
                                        }
                                    }
                                }
                            }
            ...
            {
                Console.WriteLine($"提取教师ID时出错: {ex.Message}");
            }
            
            Console.WriteLine($"共找到 {teacherMap.Count} 个教师ID映射");
            return teacherMap;
        }
        /// <summary>
        /// 解析课程学分信息
        /// </summary>
        public static Dictionary<string, double> ParseCourseCredits(HtmlDocument doc)
        {
                                    ...
                                    try
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(courseNumber) && creditsColIndex >= 0)
                                        {
                                            ...
                                            if (double.TryParse(creditsText, out double credits))
                                            {
                                                ...
                                                Console.WriteLine($"提取到课程 {courseNumber} 的学分: {credits}");
                                            }
                                        }
                                    }
                                    catch (Exception ex)
                                    {
                                        Console.WriteLine($"解析学分时出错: {ex.Message}");
                                    }
                                }
                            }
                        }
                        ...
                        try
                        {
                            ...
                            if (courseNumberMatch.Success)
                            {
                                ...
                                if (double.TryParse(credits, out double creditsValue))
                                {
                                    ...
                                    Console.WriteLine($"从JS代码提取到课程 {courseNumber} 的学分: {creditsValue}");
                                }
                            }
                        }
                        catch (Exception ex)
                        {
                            Console.WriteLine($"从JS提取学分时出错: {ex.Message}");
                        }
                    }
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"使用正则提取学分信息时出错: {ex.Message}");
                }
            }
            
            Console.WriteLine($"共提取到 {result.Count} 条课程学分信息");
            return result;
        }
        ...
        /// 从课程表表格提取教师和课程关联信息
        /// </summary>
        public static Dictionary<string, TeacherCourseInfo> ExtractTeacherCourseFromTable(HtmlDocument doc)
        {
            ...
            try
            {
                            ...
                            foreach (var row in rows)
                            {
                                ...
                                if (courseLink != null)
                                {
                                    ...
                                    if (match.Success && !string.IsNullOrEmpty(courseNumber))
                                    {
                                        ...
                                        Console.WriteLine($"从表格提取: 课程={courseNumber}, 教师={teacherName}, LessonId={lessonId}");
                                    }
                                }
                            }
                        }
                ...
                Console.WriteLine($"从表格提取了 {result.Count} 条教师-课程对应关系");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"从表格提取教师信息出错: {ex.Message}");
            }
        ...
        /// 提取课程名称（去除序号部分）
        /// </summary>
        public static string ExtractCourseName(string fullName)
        {
            // 检查输入是否为空
        ...
        /// 处理周次模式字符串
        /// </summary>
        public static string ProcessWeekPattern(string weekPattern)
        {
            // 反转字符串并移除前导零
        ...
        /// 从周次模式中获取上课周次列表
        /// </summary>
        public static List<int> GetWeeksFromPattern(string weekPattern)
        {
        ...
        /// 合并或添加课程，增强跨行处理和数据补充
        /// </summary>
        public static void MergeOrAddCourse(List<CourseInfo> courses, CourseInfo newCourse)
        {
            // 改进匹配逻辑
            ...
            if (existingCourse != null)
            {
                ...
                Console.WriteLine($"合并课程: {existingCourse.CourseName}, 节次数量: {existingCourse.Periods.Count}");
            }
            else
            {
                // 添加新课程
                courses.Add(newCourse);
                Console.WriteLine($"添加新课程: {newCourse.CourseName}, 节次数量: {newCourse.Periods.Count}");
            }
        }
        ...
        /// 最终合并清理，识别并合并冗余的课程信息
        /// </summary>
        public static List<CourseInfo> MergeDuplicateCourses(List<CourseInfo> courses)
        {
            Console.WriteLine("开始最终合并处理，删除重复课程...");
            ...
            foreach (var course in orderedCourses)
            {
                ...
                {
                    Console.WriteLine($"跳过截断课程名: {course.CourseName}");
                    continue;
                }
                ...
                {
                    Console.WriteLine($"跳过重复课程: {courseKey}");
                    continue;
                }
            ...
            Console.WriteLine($"最终合并完成，从 {courses.Count} 减少到 {result.Count} 门课程");
            return result;
        }
        ...
        /// 检查两组节次是否有重叠
        /// </summary>
        public static bool HaveOverlappingPeriods(List<int> periods1, List<int> periods2)
        {
            // 任一为空则检查另一方是否为空
        ...
        /// 合并两个课程的信息到主课程
        /// </summary>
        public static void MergeCoursesData(CourseInfo main, CourseInfo other)
        {
            // 合并基本属性，用非空值替换空值
        ...
        /// </summary>
        public static List<CourseInfo> ParseCourseInfoAlternative(string htmlContent, HtmlDocument doc, int unitCount, Dictionary<string, double> courseCredits)
        {
            List<CourseInfo> courses = new List<CourseInfo>();
            ...
            try
            {
                ...
                Console.WriteLine($"备用解析方法找到 {activityBlocks.Count} 个课程活动");
                ...
                foreach (Match block in activityBlocks)
                {
                    ...
                    {
                        Console.WriteLine($"备用解析方法处理课程活动时出错: {ex.Message}");
                    }
                }
                ...
                if (courses.Count == 0)
                {
                    Console.WriteLine("尝试从课表表格直接解析课程信息");
                        ...
                        foreach (var table in tables)
                        {
                            ...
                            if (cells != null)
                            {
                                foreach (var cell in cells)
                                {
                                    ...
                                    {
                                        Console.WriteLine($"解析表格单元格信息时出错: {ex.Message}");
                                    }
                                }
                            }
                        }
                ...
                Console.WriteLine($"备用解析方法共解析出 {courses.Count} 门课程");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"备用解析方法出现错误: {ex.Message}");
            }
        ...
        /// 解析表格中的跨行课程信息
        /// </summary>
        public static void ProcessRowspanCourses(List<CourseInfo> courses, HtmlDocument doc, int unitCount)
        {
            try
            {
                Console.WriteLine("开始处理跨行课程信息...");
                ...
                foreach (var table in tables)
                {
                    ...
                    {
                        Console.WriteLine("未找到跨行单元格");
                        continue;
                    }
                    
                    Console.WriteLine($"找到 {rowspanCells.Count} 个跨行单元格");
                        ...
                        try
                        {
                                ...
                                int.TryParse(idParts[1], out int tableIndex))
                            {
                                ...
                                if (courseMatch.Success)
                                {
                                    ...
                                    // 添加或合并到课程列表
                                    MergeOrAddCourse(courses, course);
                                    Console.WriteLine($"处理跨行课程: {courseName}, 在星期{dayOfWeek}，第{period+1}节开始，共{rowspan}节");
                                }
                            }
                        }
                        catch (Exception ex)
                        {
                            Console.WriteLine($"处理跨行单元格出错: {ex.Message}");
                        }
                    }
                }
            }
            ...
            {
                Console.WriteLine($"处理跨行课程时发生错误: {ex.Message}");
            }
        }
}
